...
# EMVA1288 parameters
path = os.getcwd()
filename = "EMVA1288_Data_dark_spatial.txt"
...
#set exposure time at Vsat/2
api.set_control_value("exposure", exposure_vsat_2)
time.sleep(0.2)

write_file(path, filename, "d " + str(exposure_vsat_2 / 1000))

for nimage in range(1,spatial_nframe+1+drop_frame):
    if (nimage > drop_frame and int(exposure_vsat_2 / 1000) < 10):
        write_file(path, filename, "i Images\\d_s_000_snap_00" + str(nimage - drop_frame) + ".tif")
        take_image("d_s_000_snap_00{}".format(str(nimage - drop_frame)) + ".raw")
        print("saved image: d_s_000_snap_00{}.raw".format(str(nimage - drop_frame)))
    elif (nimage > drop_frame and int(exposure_vsat_2 / 1000) >= 10):
        write_file(path, filename, "i Images\\d_s_000_snap_0" + str(nimage - drop_frame) + ".tif")
        take_image("d_s_000_snap_0{}".format(str(nimage - drop_frame)) + ".raw")
        print("saved image: d_s_000_snap_0{}.raw".format(str(nimage - drop_frame)))
